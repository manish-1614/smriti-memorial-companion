...
import { X, Sparkles, Upload, Heart, Languages } from 'lucide-react';

export interface MemorialModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (profile: Omit<MemorialProfile, 'id' | 'userId' | 'createdAt' | 'updatedAt'>, id?: string) => Promise<void>;
  initialData?: MemorialProfile | null;
  companion?: MemorialProfile | null;
}
...
export default function MemorialModal({ isOpen, onClose, onSave, initialData, companion }: MemorialModalProps) {
  const activeCompanion = companion ?? initialData ?? null;

  const [name, setName] = useState('');
  const [relationship, setRelationship] = useState('');
  const [avatarType, setAvatarType] = useState<'emoji' | 'image'>('emoji');
  const [avatarValue, setAvatarValue] = useState('🕊️');
  const [selectedTraits, setSelectedTraits] = useState<string[]>(['Warm & Encouraging']);
  ...
  // Synchronize form state whenever active companion or isOpen changes
  useEffect(() => {
    if (!isOpen) return;

    if (activeCompanion) {
      // Edit Mode: pre-populate all stored companion fields
      setName(activeCompanion.name || '');
      setRelationship(activeCompanion.relationship || activeCompanion.relation || '');
      
      const isImg =
        activeCompanion.avatarType === 'image' ||
        !!activeCompanion.avatarUrl ||
        activeCompanion.avatarValue?.startsWith('data:') ||
        activeCompanion.avatarValue?.startsWith('http');
      setAvatarType(isImg ? 'image' : 'emoji');
      setAvatarValue(activeCompanion.avatarValue || activeCompanion.avatarUrl || '🕊️');

      if (activeCompanion.personalityTraits && activeCompanion.personalityTraits.length > 0) {
        setSelectedTraits(activeCompanion.personalityTraits);
      } else if (activeCompanion.personalitySeed) {
        setSelectedTraits([activeCompanion.personalitySeed]);
      } else {
        setSelectedTraits(['Warm & Encouraging']);
      }

      setToneDescription(activeCompanion.toneDescription || activeCompanion.voiceConfig || '');
      setPrimaryLanguage(activeCompanion.primaryLanguage || '');
      setBirthYear(activeCompanion.birthYear || '');
      setPassedYear(activeCompanion.passedYear || activeCompanion.yearOfPassing || '');
      setBioSnippet(activeCompanion.bioSnippet || '');
      setCustomTrait('');
      setError(null);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) {
      setError('Please provide a name for this memorial companion.');
      return;
    }
    ...
    try {
      await onSave(
        {
          ...
          avatarUrl: avatarType === 'image' ? avatarValue : undefined,
          personalityTraits: selectedTraits,
          personalitySeed: selectedTraits[0] || undefined,
          toneDescription: toneDescription.trim(),
          voiceConfig: toneDescription.trim(),
          primaryLanguage: primaryLanguage.trim() || undefined,
          birthYear: birthYear.trim(),
          passedYear: passedYear.trim(),
          yearOfPassing: passedYear.trim(),
      ...
      setError(err instanceof Error ? err.message : 'Failed to save memorial companion.');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-serif font-bold tracking-tight text-[#4A443F]">
                {activeCompanion ? 'Edit Memorial Companion' : 'Create Memorial Profile'}
              </h2>
              <p className="text-xs text-[#8C7B6E] mt-0.5">
                Record the cherished persona, voice, and essence of your loved one.
              </p>
            </div>
          </div>
          <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#8C7B6E]">
                Memorial Avatar (Photo or Symbol)
              </label>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full pl-8 pr-3.5 py-2.5 rounded-2xl bg-white border border-[#E5E0D5] text-[#4A443F] placeholder-[#A69F95] focus:outline-hidden focus:ring-2 focus:ring-[#7D8F69]/30 text-sm shadow-xs"
                  />
                  <Languages className="w-3.5 h-3.5 text-[#A69F95] absolute left-3 top-3" />
                </div>
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-[#8C7B6E] mb-1.5">
                Voice &amp; Tone Nuance / Personality Seed (Optional)
              </label>
              <input
                type="text"
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    {activeCompanion ? 'Save Changes' : 'Create Memorial'}
                  </>
                )}
              </button>
            </div>
...
export { MemorialModal as EditMemberModal };
